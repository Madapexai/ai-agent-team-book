...
- Agent collusion is a hidden risk — an independent Inspector Agent + honeypot tasks are effective defenses
...
![Image "What to do when your Agent messes up · failure modes and recovery". Under the title, it shows typical Agent failure modes and the recovery flowchart. Failure modes include making up non-existent APIs/functions, fake code; destroying source files, modifying code that shouldn't be changed; infinite loops, going in circles on the same problem, unable to break free; as it works it forgets the original goal, drifting further off; one Agent's error triggers a chain reaction; after failure it goes silent, doesn't report, pretends to succeed, etc., accounting for 23%, average detection time 12min. The recovery flow covers detection, pause, analysis, correction, verification, learning, etc., 94% auto-recoverable, average recovery time <5min.](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NjhkMmQ1NWFlYTFkNGMwNDc4Zjg4ZjgwMDZkYzg4ZWJfOWY3YzU1N2JkYWQxOGJiZjA0YWRlNDk3MTI1YmNjYjdfSUQ6NzY1MTgwMDE0NTU5NzgyODA0Nl8xNzgzODcwNTIxOjE3ODM4NzQxMjFfVjM)
