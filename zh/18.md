![图片展示了子Agent架构，以“Supervisor Agent”为核心，周围分布着多个子Agent，如Review Agent、Testing Agent等，每个子Agent下方标注了数量，如Supervisor为1、Main Agent为4、子Agent总数为11。图片下方文字说明子Agent架构的威力在于“分而治之”——每个子Agent专注一个领域，组合起来就是全线团队。该图与上下文紧密相关，直观呈现了文档中提到的子Agent架构，帮助理解文档中Kai因上下文窗口不够而卡住的问题。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZGQyMjYxNDNlMmIxNzljNTQzMGU1YjFhZTFiYTUxZDZfNzczMGFmZTZiZjZlNGRmNjA1MmIyZGZlZjY2ZmUyNWNfSUQ6NzY1MTgwMDE2MjQxOTM0NjQwM18xNzgzODcwNTIyOjE3ODM4NzQxMjJfVjM)
...
这个设计是Yason的Agent团队能从3个Agent扩展到10个Agent的关键。如果父Agent存储所有子Agent的完整历史，上下文窗口会以O(n)的速度膨胀——每增加一个子Agent，父Agent的Token消耗就翻倍。有了上下文隔离，父Agent的Token消耗是O(1)——无论多少个子Agent，父Agent只存意图和摘要。
...
![图片展示了子Agent架构，以“Supervisor Agent”为核心，周围分布着多个子Agent，如“Review Agent”“Testing Agent”等。每个子Agent下方标注了其数量，如“](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZGIyNmFmZmFlNzMyOWY3NzgyMWI3MDFkMTIzMmE3NWZfNDczN2E2ZWY3ZjU3ZTk5OTc0MWRiZDA1YTllNTBiMmVfSUQ6NzY1MTgwMDE2NDIxODM3NTE1MV8xNzgzODcwNTIyOjE3ODM4NzQxMjJfVjM)