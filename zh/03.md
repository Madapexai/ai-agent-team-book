![图片展示了搭建Agent团队的真实成本。左侧为API/Token消耗，包括推理模型（Claude/GPT）0.15 - 9.0/M tokens、快速模型（Haiku/GPT - 4o - mini）0.03 - 0.15/M、Embedding / 向量0.02/M，以及向量数据库/存储、CI/CD / 部署的费用。右侧是时间投入，涵盖搭建期（2 - 4周）、前孵化期（1 - 2月）、后续孵化（每周2 - 4h）等阶段。底部有“$200 - 800”字样。该图与上下文紧密相关，直观呈现了搭建Agent团队在成本方面的具体构成。](assets/diagrams/zh_03_1.svg)
