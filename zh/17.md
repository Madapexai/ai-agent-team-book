![图片展示了多Agent协作协议栈的层次结构。从上至下依次为紧急协议、协商协议、状态协议、通信协议和任务任务协议。紧急协议用于处理无法处理的异常；协商协议是Agent间沟通协议；状态协议让每个Agent广播状态等；通信协议是Agent间消息传递的底层规范；任务协议是任务创建、分配、执行、完成的标准格式和流程。图片与上下文紧密相关，直观呈现了Yason设计协议栈时借鉴的Claude Code Agent Teams架构。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NGM0NGQ3ODk1N2QyNjQwZDJmOGQ3N2YxOTYzYzI5NDhfZWFlNjJjNjM2MDE0MjczMDY1NGEzOTZjM2I1OWY0MzNfSUQ6NzY1MTgwMDE1NjAwNTc5NjgwNF8xNzgzODcwNTIxOjE3ODM4NzQxMjFfVjM)
...
另一个让Yason眼前一亮的设计是OpenAI的Codex Subagents模式。
...
| 研究分析 | Swarm | Codex Subagents | 2-8 Agent | 隔离沙箱并行实验，PR合并结果 |
...
- 三层协议栈：通道层 → 格式层 → 协商层，缺一不可
- 工业界已形成三大方案：Claude Code Teams / OpenAI Codex Subagents / Kimi Swarm
...
![图片展示了多Agent协作协议栈的层次结构。从上至下依次为紧急协议、协商协议、状态协议、通信协议和任务协议。紧急协议用于处理无法无法处理的异常；协商协议是Agent间沟通的协议；状态协议让每个Agent广播状态等；通信协议是消息传递的底层规范；任务协议是任务创建等的标准格式和流程。图片与上下文紧密相关，直观呈现了协议栈各层](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=Yjk0YjBhODZhNDQ1ODQ3MjAyZmIxZTQ5NDljMTdhZTRfNzczNDgwMzVmNmJlMGYyMGQ0MjEzMjZmZmVkOTliMjhfSUQ6NzY1MTgwMDE1NjM0NTUwMjY5MF8xNzgzODcwNTIxOjE3ODM4NzQxMjFfVjM)