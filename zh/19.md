![图片展示了接入外部工具 - MCP集成架构。MCP协议层是所有Agent与外部世界对话的统一语言，连接一切。Agent Runtime负责工具发现、连接、调用、结果解析等。MCP Server负责工具注册、鉴权、限流、日志等。图中还列出了多个工具类别，如GitHub、数据库、云服务、项目管理、通信、向量DB等，以及监控、认证、文档、测试、容器、CI/CD、客服、AI模型等工具，直观呈现了MCP的集成架构。](assets/diagrams/zh_19_1.svg)
...
![图片展示了接入外部工具的MCP集成架构。MCP协议层是Agent与外部世界对话的统一语言，连接一切。Agent Runtime负责工具发现、调用、结果解析等。MCP Server管理工具注册、版本、期限、日志等。左侧有GitHub、数据库、云服务等工具，右侧有监控、认证、文档、测试等服务。底部文字强调MCP是所有Agent与外部世界对话的统一语言。该图与上下文介绍的MCP协议层、1. 用于统一Agent与工具的交互标准，实现Agent与工具的高效连接和协作。](assets/diagrams/zh_19_2.svg)