![图片展示了Agent故障模式与恢复流程。左侧为典型故障模式，包括编造不存在的API/函数、假代码，破坏源文件、修改不修改的代码，无限循环、同一个问题反复打转、无法跳出，随着做着忘了原始目标、越偏越远，一个Agent的错误引发连锁反应，失败后静默、不报告、假装成功，占比23%，平均发现时间为12min。右侧是恢复流程，包括检测、暂停受影响Agent、分析、修正、重试等步骤，94%的自动恢复率，平均恢复时间<5min。](assets/diagrams/zh_16_1.svg)
...
![图片“Agent搞砸了怎么办 · 故障 addCriterion故障模式与恢复”标题下，展示了Agent典型故障模式及恢复流程。故障模式包括编造不存在的API/函数、假代码，破坏源代码文件、修改不修改的代码，无限循环、同一个问题反复打转，无法跳出，随着做着忘了原始目标、越偏越远，一个Agent的错误引发连锁反应，失败后静默 addCriterion默、不报告、假装成功等，占比23%，平均发现时间为12min。恢复流程涵盖检测、暂停、分析、修正、验证、学习等步骤，94%可自动恢复，平均恢复时间<5min。](assets/diagrams/zh_16_2.svg)