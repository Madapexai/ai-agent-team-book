...
![图片"Agent搞砸了怎么办 · 故障模式与恢复"标题下，展示了 Agent 典型故障模式及恢复流程。故障模式包括编造不存在的 API/函数、假代码，破坏源代码文件、修改不该改的代码，无限循环、同一个问题反复打转无法跳出，做着做着忘了原始目标越偏越远，一个 Agent 的错误引发连锁反应，失败后静默、不报告、假装成功等，占比 23%，平均发现时间 12min。恢复流程涵盖检测、暂停、分析、修正、验证、学习等步骤，94% 可自动恢复，平均恢复时间 <5min。](assets/diagrams/zh_16_2.svg)