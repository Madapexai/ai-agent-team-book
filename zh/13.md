![图片是成本监控仪表盘，标题为“别让API账单爆掉”。内容包括四个板块：今日消耗板块显示推理模型、快速模型、Embedding的Token使用量及今日费用为$12.45；7天趋势板块列出周一至周五每天费用；本月累计费用为$287.30；预算控制板块显示月预算$5000，已用$287.30（占57%），剩余$212.70；告警和优化板块给出Token使用正常、推理模型占比高、预算剩余合理等提示。该图与文档中Yason关注API账单成本的内容相关。](assets/diagrams/zh_13_1.svg)
