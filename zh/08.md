![图片展示了混合通信体系，分为CLI模式（结构化）、NL模式（自然语言）和混合模式。CLI模式适用于Agent间Routine交互、状态同步、任务分配等场景，优势为精准、可脚本化、低Token消耗等。NL模式适用需求讨论、Bug描述等场景，优势是表达丰富、隐含信息可传递等。混合模式是最佳实践，实现方式为CLI发送结构化请求 + NL附上下文说明等。图片与上下文紧密相关，直观呈现了不同模式的特点及适用场景。](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=NGJmNTYzMTI2N2JmMWNmYjM0ODE4ZGQyNGZhMjk2MDlfNmY5YWRmNWQwZDEyMmQxMDkwYWY4NTU2N2I4NTgzYzlfSUQ6NzY1MTgwMDA3NTE4OTk2Mzc1NV8xNzgzODcwNDk1OjE3ODM4NzQwOTVfVjM)
...
![图片展示了混合通信体系，分为从左侧起分别为CLI模式（结构化）、NL模式（自然语言）和混合模式。CLI模式适用于Agent间Routine交互，状态同步、任务分配等ylko](https://internal-api-drive-stream.feishu.cn/space/api/box/stream/download/authcode/?code=ZWVjYWExMTNmYmFhNzM1ZWVhMjJiMDJlZGM3YzMxOTVfMTY3YTM3OGIzMDlmYTc5NWJiYjcyNjljZjY1N2E1NDdfSUQ6NzY1MTgwMDA3MTY4ODI2MDU3Ml8xNzgzODcwNDk1OjE3ODM4NzQwOTVfVjM)