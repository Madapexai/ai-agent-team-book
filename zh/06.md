![图片展示了“分工的艺术·Agent角色体系”，包含Coding Agent、Review Agent、Testing Agent三种角色。Coding Agent专职编码实现等；Review Agent负责代码审查等；Testing Agent执行测试全流程自动化等。各角色有明确边界，如Review Agent不碰代码只提建议，Testing Agent不决定测试策略。图片与上文运维Agent Rex越界修改Kai代码的案例相呼应，强调了明确Agent角色分工和边界的重要性。](assets/diagrams/zh_06_1.svg)
...
![图片展示了Agent角色体系中的Coding Agent、Review Agent和Testing Agent。在Coding Agent处，其职责涵盖代码实现及更多内容——完整拆解了团队中每个角色的职责范围与边界。](assets/diagrams/zh_06_2.svg)