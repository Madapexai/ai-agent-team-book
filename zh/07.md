![图片展示了让Agent认识彼此的共享记忆架构。中心为共享记忆仓库（Git），包含 structured-memory/、decisions/ 等目录，有 git commit & push、git pull & merge 操作。四周分布着 Coding Agent、Review Agent 等不同 Agent，分别负责写入、读取等任务。底部说明共享记忆为Git仓库持久化、Markdown数据结构化，Agent们在同一个知识空间协作。该图与上下文紧密相关，直观呈现了文档中提到的Agent如何通过共享记忆进行协作。](assets/diagrams/zh_07_1.svg)
...
![图片展示了让 Agent 认识彼此的共享记忆架构。中心为共享记忆仓库（Git），包含 structured-memory/、decisions/、progress/、context/、references/ 等目录，有 git commit & push、git pull & merge 操作。周围是多个 Agent——Coding Agent、Review Agent、Testing Agent、Docs Agent、DevOps Agent、Supervisor、Research Agent、Future Agents，分别负责读写记忆、依赖记忆上下文、获取测试策略、提取变更、写入部署状态、全局协调、归档调研结果、记忆 API 等。](assets/diagrams/zh_07_2.svg)