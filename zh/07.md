![图片展示了让Agent认识彼此的共享记忆架构。中心为共享记忆仓库（Git），包含structured - memory/、decisions/等目录，有git commit & push、git pull & merge操作。四周分布着Coding Agent、Reviewview Agent等不同Agent，分别负责写入、读取等任务。底部说明共享记忆为Git仓库持久化、Markdown数据结构化，Agent们在同一个知识空间协作。该图与上下文紧密相关，直观呈现了文档中提到的Agent如何通过共享记忆进行协作。](assets/diagrams/zh_07_1.svg)
