![图片展示了Agent自进化机制的四个阶段。阶段1为观察，通过执行任务记录成功/失败模式、Token消耗审计；阶段2为分析，识别流程模式、发现改进机会、对比历史表现；阶段3为改进，优化Prompt、调整工作流程、更新知识库；阶段4为验证，A/B测试验证、效果量化评估、迭代循环。每24小时一次自动进化循环，人类仅需审批改进提案。图片与上下文紧密相关，直观呈现了Agent自进化的工作流程。](assets/diagrams/zh_12_1.svg)
...
![图片展示了Agent自进化机制的四个阶段。阶段1为观察，通过执行任务记录成功/失败模式、Token消耗审计；阶段2为分析，识别成功模式、发现改进机会；阶段3为改进；阶段4为验证，通过A/B测试验证。](assets/diagrams/zh_12_2.svg)